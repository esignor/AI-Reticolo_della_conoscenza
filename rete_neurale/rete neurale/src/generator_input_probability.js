...
generator_input_probability = function(dim_vettore) {
    ...
    while (i < dim_vettore - numero_zero) {
        ...
        if (vector[posizione_random] == null) { // controllo che la cella non sia gia' occupata
            ...
            /**
             * Regole: Per una domanda va valutata anche la domanda sorella. Se entrambe hanno lo stesso valore allora nel caso di -1 i genitori saranno settati
             * a -1, se hanno entrambe valore 1 allora i figli saranno settati a 1. Tutto questo viene calcolato tenedo presente la probabilita' effettiva per un
            ...
            if (posizione_random == 0) { // domanda 1
                ...
                if (i + numero_zero < dim_vettore && vector[3] == null) { // calcolo la probabilita' che la domanda 4 sia data in modo corretto
                    vector[3] = valore;
                    i = i + 1;
                }

                if (vector[3] == -1 && vector[0] == -1) { // sia la domanda 4 che 1 sono state date sbagliate, i padri che sono piu' complessi saranno sicuramente errati
                ...
                if (vector[2] == 1 && vector[5] == 1) {
                    ...
                    if (vector[3] == -1 && vector[0] == -1) { // i genitori sono conseguentemente sbagliati altrimenti non posso dire nulla
                        if (i + numero_zero < dim_vettore && vector[2] == null) {
                            vector[2] = -1;
                            i = i + 1;
                        }
    ...
    // posizionati in vector dim_vettore  - numero_zero elementi
    valore = 0;
...
function calcolo_risposta_probabilita() {
    ...
    if (!candidato_scarto1) { // non ne sa scartare nessuna, calcolo il fattore fortuna
        if (Math.floor(1 / 3 * ((Math.floor(Math.random() * 3) + 1))) == 0) // ha una possibilta' su 3 di azzeccare la risposta  -> 1/3
            return -1;
        else
            return 1;
    }
        ...
        if (!candidato_scarto2) { // ne sa scartare una; ma non 2
            if (Math.floor(1 / 2 * ((Math.floor(Math.random() * 2) + 1))) == 0)// ha 1 possibilita' su 2 di saper rispondere giusto + il fattore fortuna -> 1/3 + 1/6 s1 -> s1 = 1 allora 1/3 + 1/6 = (2+1)/6 = 1/2
                return -1
            else
                return 1;
        }
